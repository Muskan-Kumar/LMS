...
const CourseDetails = () => {
  ...
  return courseData ? (
    ...
    justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
      <div className='absolute top-0 left-0 w-full h-[500]px -z-[1] bg-gradient-to-b from-cyan-100/70'></div>

        {/* left column */}
        <div className='max-w-xl z-10 text-gray-500'>
          <h1 className='md:text-[56px] text-course-details-heading-small font-semibold text-gray-800'>{courseData.courseTitle}</h1>
          ...
          <div className='flex items-center space-x-2'>
            ...
            <p className='text-blue-600'>({courseData.courseRating ? courseData?.courseRating?.length : 0} {(courseData?.courseRating?.length || 0) > 1 ? 'ratings':'rating'} )</p>
            <p className=''> {(courseData?.enrolledStudents?.length ||0)} {(courseData?.enrolledStudents?.length ||0) > 1 ? 'students' : 'student'} </p>
          </div>
          <p className='text-sm'>Course by <span className='text-blue-600 underline'>{course.educator.name}</span></p>
        ...
        <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
          ...
          <div className='p-5'>
            ...
            >{isAlreadyEnrolled ? 'Already Enroll' : 'Enroll Now'} </button>
